...
const chatNotificationsEnabled=false,chatLocationBusy=false,sW=true,sQ=true,sY=true;const syncApsoChatMessages=()=>{},toggleChatNotifications=()=>{},setTaskAssignModalOpen=()=>{},openDriveHelper=()=>{},handleSendChatLocation=()=>{},handleChatFileInput=()=>{},setChatMentionRefs=()=>{},setChatResidentRefs=()=>{},downloadLocalFile=()=>{},setPreviewImageModal=()=>{},handleUpdateChatTaskStatus=()=>{},tT=()=>{},openResidentFromChat=id=>window.openedResident=id;
function sendApsoChatMessage(p){window.sent=p;setChatMessages(v=>[...v,{...p,id:'m'+v.length,senderId:'u1',senderName:'Hồng',createdAt:Date.now(),expiresAt:Date.now()+999999}])}
...
await page.screenshot({path:'tmp/chat-real-mobile.png'});assert.deepEqual(errors,[]);console.log('PASS actual chat JSX: mobile render, @ unaccented multiword selection, Enter does not send, # links open exact record');
